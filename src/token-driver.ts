...
export const READ_ONLY_TOKEN_CAPABILITIES: CdpChatCapabilities = {
  list_chats: true,
  search_chat: true,
  export_chat: true,
  new_chat: false,
...
interface ConversationPayload {
  ...
  mapping?: Record<string, { message?: ConversationNodeMessage }>;
}
...
class BridgeTransport {
  ...
  private readonly pending: Array<(envelope: BridgeEnvelope) => void> = [];
  private buffer = "";
  ...
  request(url: string, headers: Record<string, string>): Promise<BridgeEnvelope> {
    return new Promise<BridgeEnvelope>((resolve, reject) => {
      ...
      const handler = (envelope: BridgeEnvelope) => {
    ...
    this.child.stdout?.on("data", (chunk: string) => {
      ...
      while (index >= 0) {
        ...
        if (line) {
          ...
          if (handler) {
            try {
              handler(JSON.parse(line) as BridgeEnvelope);
            } catch {
...
function epochMs(value: number | string | undefined): number {
  if (typeof value === "string" && value.trim()) return Date.parse(value) || 0;
...
export class TokenCdpChatPage implements CdpChatPage {
  ...
  constructor(
    ...
    private readonly options: {
      accountId?: string; fetchImpl?: typeof fetch; fetchCommand?: string;
      snapshotDepth?: number; detailDelayMs?: number;
    },
    private readonly bridge: BridgeTransport | null,
  ) {
  ...
  async snapshot(): Promise<ChatSnapshot> {
    ...
    const list = await this.httpGetJson<{ items?: ConversationListItem[] }>(
      `/backend-api/conversations?offset=0&limit=100&order=updated&is_archived=false`,
    ...
    const detailed = new Map<string, ConversationPayload>();
    for (const item of items.slice(0, depth)) {
      if (delay > 0) await new Promise((resolve) => setTimeout(resolve, delay));
      try {
        detailed.set(item.id, await this.httpGetJson<ConversationPayload>(
          `/backend-api/conversation/${encodeURIComponent(item.id)}`,
          { Authorization: `Bearer ${token}` },
        ));
      } catch {
    ...
    const chats: ChatRecord[] = items.map((item) => {
      const conversation = detailed.get(item.id);
      const mapping = conversation?.mapping ?? {};
      const messages: MessageRecord[] = Object.values(mapping)
        ...
        .map((message) => {
          ...
          const text = parts
            .map((part) => (typeof part === "string" ? part : `[${String((part as { content_type?: string }).content_type ?? "asset")}]`))
            .join(" ")
            .replace(/\s+/g, " ")
            .trim();
          return {
  ...
  async sendMessage(): Promise<MessageRecord> {
  ...
  private async ensureToken(): Promise<string> {
    ...
    const session = await this.httpGetJson<{ accessToken?: string; user?: { id?: string } }>(
      "/api/auth/session", {}, true,
    );
    const token = String(session.accessToken ?? "");
    if (!token) throw new Error("session cookie was rejected: no accessToken in /api/auth/session");
    this.accessToken = token;
    return token;
  }

  private async httpGetJson<T>(
    path: string, headers: Record<string, string> = {}, sessionAuth = false,
  ): Promise<T> {
...
function readSessionToken(options: TokenDriverOptions): string {
  ...
  if (file.trim()) {
    const state = JSON.parse(readFileSync(file.trim(), "utf-8")) as Record<string, unknown>;
    const token = String(state.session_token ?? state.sessionToken ?? "").trim();
    if (token) return token;
  }
  throw new Error(
    "token driver requires CHATGPT_TOKEN_SESSION_TOKEN or CHATGPT_TOKEN_SESSION_FILE",
  );
}

export async function createTokenCdpChatDriver(
  options: TokenDriverOptions = {},
): Promise<CdpChatDriver & { capabilities: CdpChatCapabilities }> {
  const depth = Number.parseInt(process.env.CHATGPT_TOKEN_SNAPSHOT_DEPTH ?? "", 10);
  const resolved: TokenDriverOptions = {
    ...options,
